...
def signup(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.info(request, message="Your account has just been created, you can login after an admin has allowed you to!")
...
def create_group(request):
    ...
    if request.method == "POST":
        ...
        for key in request_dict.keys():
            dict_element = request_dict.get(str(key))
            if isinstance(dict_element, list) and str(dict_element[0]).strip() == "":
                messages.warning(request, message=f"Cannot create a group with Empty fields, try again!")
                return redirect('core_app:create_group')
            if 'projectmember' in str(key).lower():
                print(f"\nA project member : {type(request_dict.get(str(key)))}\n")
                member = request.POST[str(key)]
                group_members.append(str(member))
            if 'member_reg_num' in str(key).lower():
                print(f"\nA member's reg num : {type(request_dict.get(str(key)))}\n")
                member_reg_num = request.POST[str(key)]
                members_reg_nums.append(str(member_reg_num))

        if len(group_members) != len(members_reg_nums) or len(group_members) == 0:
        ...
        try:
            ...
            for name, reg_num in zip(group_members, members_reg_nums):
                if str(name).strip() == "" or str(reg_num).strip() == "":
                    messages.warning(request, message=f"Can't save members with empty names or Registration numbers !")
                    return redirect('core_app:create_group')
                if GroupMember.objects.filter(regnumber=reg_num):
                    ...
                    return redirect('core_app:create_group')
            new_group = ProjectGroup(name=group_name, faculty=group_faculty, academic_year=academic_year )
            new_group.save()
            
            for name, reg_num in zip(group_members, members_reg_nums):
                member = GroupMember(name=name, regnumber=reg_num, project_group=new_group)
                member.save()
...
@dynamic_login_required
def create_single_member(request, group_id):
    try:
        if request.method == "POST":
            member_name = request.POST["member_name"]
            member_reg_num = request.POST['reg_num']
            if (isinstance(member_name, list) and str(member_name[0]).strip() == "") or (isinstance(member_reg_num, list) and str(member_reg_num[0]).strip() == ""):
                messages.warning(request, message=f"Can't save members with empty names or Registration numbers !")
                return redirect('core_app:single_group', group_id=group_id)
            try:
                if GroupMember.objects.filter(regnumber=member_reg_num.strip()):
                    messages.warning(request, message=f"Student with Registration Number {member_reg_num} already exists, try again!")
                    return redirect('core_app:single_group', group_id=group_id)
                # if 
                member_group = ProjectGroup.objects.get(pk=group_id)
                member = GroupMember(name=member_name.strip(), regnumber=member_reg_num.strip(), project_group=member_group)
                member.save()
                return redirect('core_app:single_group', group_id=group_id)
            except ProjectGroup.DoesNotExist:
                return render(request, 'error_page.html', {'error_code':'404', 'error_message':"Oops! This page does not exist!"})
    except Exception as e:
        print(f"\nException in create_single_member as : {e}\n")
        return render(request, 'error_page.html', {'error_code':'404', 'error_message':"Oops! Something went wrong !"})
@dynamic_login_required
